...
<div class="row">
    <div class="col-md-12 col-sm-12  animation">
        <div class="x_panel">
          ...
          <div class="x_content">
            <form id="demo-form" action="{{route('balance-transfer.store')}}" method="Post">
              @csrf
              
              <div class="row">
                <div class="col-12 mb-3">
                  <label for="reasone">Transfer Reason * :</label>
                  <input type="text" id="reasone" class="form-control" value="{{old('transfer_reason')}}" name="transfer_reason" class="@error('transfer_reason') is-invalid @enderror">
                ...
                <div class="col-6 mb-3">
                  ...
                  <select id="from-account" class="form-control js-example-basic-single" class="@error('from_account_id') is-invalid @enderror" name="from_account_id">
                    @foreach ($accounts as $account)
                        <option value="{{$account->id}}">{{$account->bank_name}} {{$account->account_number ? '[ '.$account->account_number.' ]' : ''}}</option>
                    @endforeach
                  </select>
                </div>
                ...
                <div class="col-6 mb-3">
                  ...
                  <input type="number" id="ammount" class="form-control" name="ammount" value="{{old('ammount')}}" class="@error('ammount') is-invalid @enderror">
                  @error('ammount')
                      <span class="text-danger">{{ $message }}</span>
                  @enderror
                </div>
