...
<div class="row">
    <div class="col-md-12 col-sm-12  animation">
        <div class="x_panel">
          ...
          <div class="x_content">
            <form id="demo-form" action="{{route('expense.expenses.update',$expense->id)}}" method="POST">
              @csrf
              @method('PUT')
              ...
              <div class="row">
                <div class="col-12 mb-3">
                  ...
                  <input type="text" id="reason" value="{{$expense->reason}}" class="form-control" name="reason" class="@error('reason') is-invalid @enderror">
                  @error('reason')
                ...
                <div class="col-6">
                    ...
                    <select id="category_id" class="form-control form-control @error('category_id') is-invalid @enderror" name="category_id">
                      ...
                      @foreach ($categories as $category)
                          <option {{$category->id === $expense->exp_category_id ? 'selected' : '' }} value="{{$category->id}}">{{$category->name}}</option>
                      @endforeach
                  </select>
                </div>
                ...
                <div class="col-6 mt-3">
                    ...
                    <select id="account_id" class="form-control form-control @error('account_id') is-invalid @enderror" name="account_id">
                      ...
                      @foreach ($accounts as $account)
                          <option {{$account->id === $expense->account_id ? 'selected' : '' }} value="{{$account->id}}">{{$account->bank_name}}</option>
                      @endforeach
                  </select>
                </div>
                ...
                <div class="col-4 mt-3">
                  ...
                  <input type="number" id="ammount" class="form-control" value="{{$expense->ammount}}" name="ammount" class="@error('ammount') is-invalid @enderror">
                  @error('ammount')
                      <span class="text-danger">{{ $message }}</span>
                  @enderror
                </div>
                ...
                <div class="col-4 mt-3">
                  ...
                  <input type="number" id="check_no" class="form-control" value="{{$expense->check_no}}" name="check_no" class="@error('check_no') is-invalid @enderror">
                  @error('check_no')
                      <span class="text-danger">{{ $message }}</span>
                  @enderror
                </div>
                ...
                <div class="col-4 mt-3">
                  ...
                  <input type="text" id="voucher_no" class="form-control" value="{{$expense->voucher_no}}" name="voucher_no" class="@error('voucher_no') is-invalid @enderror">
                  @error('voucher_no')
                      <span class="text-danger">{{ $message }}</span>
                  @enderror
                </div>
                ...
                <div class="col-12 mt-3">
                  <label for="message">Note (100 chars max) :</label>
                    <textarea id="message" class="form-control form-control" value="{{$expense->note}}" name="note" class="@error('note') is-invalid @enderror"></textarea>
                    @error('note')
                      <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="col-6 mt-3">
                  ...
                  <input type="date" id="date" class="form-control" value="{{$expense->date}}" name="date" class="@error('date') is-invalid @enderror">
                  @error('date')
                      <span class="text-danger">{{ $message }}</span>
                  @enderror
                </div>
    ...
    <script>
      $(document).ready(function(){
        ...
        $("#account_id").change(function(){
            id = $(this).val();
            checkavailableBalance(id);
        });

        checkavailableBalance(id);

        function checkavailableBalance(id){
          $.ajax({
